...
query = f'Field of Interest - {q_field}, Desired Position - {q_desired}, Desired Companies - {q_companies}, Desired Industry - {q_meta}'
v2_prompt = f" are tasked with building a career path for an entry-level linkedin user who is interested in the field of {q_field} and wants to be a {q_desired}. We add self-descriptions of linkedin profiles which are relevant to the query-submitter's goals here, use infromation from this in your output - {context}, Output should be a list of up to {n_steps} steps for the user to take, each is explained fully and contains concrete examples."
