...
scraped = spark.read.json(LOCAL_DATA + "/j_lu9lrj1v25cws1yhu9.json", multiLine=True)
...
def get_new_columns(original_df, new_df):
    """ Returns a list of the columns that are in new_df but not in original_df. """
...
# MAGIC %md
# MAGIC We can also see that some of the schema wasn't inferred correctly. We can fix this because the data is the same data, It's just a format issue.
...
scraped = spark.read.json(path=LOCAL_DATA + "/j_lu9lrj1v25cws1yhu9.json", multiLine=True, schema=true_schema)
scraped.write.mode("overwrite").parquet(LOCAL_DATA + "/scraped_leaders")
...
def ignore_brackets_split(s, separator=','):
    """
        ...
        Splits the given string by given separator, but ignores separators inside brackets pairs, e.g.
...
comparisons = original_leaders.join(other=scraped.select("id", *[F.col(c).alias(c + "_scraped") for c in scraped.columns if c != "id"]), 
                                    on="id")
display(comparisons)
...
for c in ["position", "current_company:company_id", "current_company:name", "about"]:
    condition = condition | (F.col(c).isNotNull() & (F.length(F.col(c)) != F.length(F.col(c + "_scraped"))))
...
condition = F.lit(False)
for c, c2 in [("certifications.meta", "certifications_scraped.meta"), 
...
# MAGIC %md
# MAGIC These results indicate that the scraping indeed gave new information about the leaders. This can definitely influence their scoring in the retrieval stage.
...
augmented_profiles = profiles.join(other=scraped.select("id", *[F.col(c).alias(c + "_scraped") for c in scraped.columns if c != "id"]), 
                                   on="id",
                                   how="left")
display(augmented_profiles)
augmented_profiles.count()

# COMMAND ----------

augmented_profiles.dtypes
...
augmented_profiles = augmented_profiles.select(*profiles.columns)
augmented_profiles.display()
